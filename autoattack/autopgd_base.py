...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        # AA default step_size = self.alpha*self.eps
        step_size = self.alpha * torch.ones([x.shape[0], *(
        ...
        x_adv_real = deepcopy(x_adv)
        x_adv_old_real = x_adv_real.clone()

        sign_op = True if self.update_method in ['sign_grad'] else False
        clip_op = True if self.update_method in ['sign_grad', 'raw_grad'] else False
        clip_mom_op = False if self.update_method in ['noclip_raw', 'rgd_aa'] else True

        for i in range(self.n_iter):
            ### gradient step
            if i == self.rgd_iter and self.update_method == 'rgd_aa':
                # we convert rgd into aa
                sign_op, clip_op, clip_mom_op = True, True, True
                x_adv_old = torch.clamp(torch.min(torch.max(x_adv_old_real,
            ...
            with torch.no_grad():
                x_adv = x_adv.detach()
                if not clip_mom_op:
                    grad2 = x_adv_real - x_adv_old_real
                    x_adv_old_real = x_adv_real.clone()
                else:
                ...
                if self.norm == 'Linf':
                    adv_update = torch.sign(grad) if sign_op else grad
                    if not clip_op:
                        x_adv_real_1 = x_adv_real + step_size * adv_update
                        x_adv_real = x_adv_real + (x_adv_real_1 - x_adv_real) * a + grad2 * (1 - a)
                        x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_real,
                            x - self.eps), x + self.eps), 0.0, 1.0)
                    else:
                        x_adv_1 = x_adv + step_size * adv_update
            ...
            with torch.no_grad():
              ...
              if counter3 == k:
                  if self.norm in ['Linf', 'L2']:
                      ...
                      step_size[fl_redtopk] = self.alpha
                      step_size[~fl_redtopk] /= adasp_redstep
                      step_size.clamp_(self.alpha / adasp_minstep, self.alpha)
        ...
        return (x_best, acc, loss_best, x_best_adv, acc_steps)
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.use_largereps:
                        ...
                        best_curr, acc_curr, loss_curr, adv_curr, acc_steps = res_curr
                    else:
            ...
            for counter in range(self.n_restarts):
                best_curr, _, loss_curr, _, acc_steps = self.attack_single_run(x, y)
