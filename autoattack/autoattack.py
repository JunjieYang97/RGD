...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 ...
                 device='cuda', log_path=None, n_iter=100, n_restarts=5, 
                 update_method = 'sign_grad', rand_init=True, alpha=2, rgd_iter=5):
        self.model = model
        self.norm = norm
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=self.n_restarts, n_iter=self.n_iter, verbose=False,
                eps=self.epsilon, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed,
                device=self.device, logger=self.logger, update_method=self.update_method, 
                rand_init = self.rand_init, alpha=self.alpha, rgd_iter=self.rgd_iter)
            ...
            for attack in attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    if attack == 'apgd-ce':
                        ...
                        adv_curr, acc_steps = self.apgd.perturb(x, y) #cheap=True
